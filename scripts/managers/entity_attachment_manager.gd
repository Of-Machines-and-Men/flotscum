...
var _queue: Array[AttachmentRequest] = []

func _is_duplicate(request: AttachmentRequest, queue_entry: AttachmentRequest):
...
func _add_queue_entry(absorber: Entity, target: Entity) -> void:
	var request = AttachmentRequest.new(absorber, target)
	for entry in _queue:
...
func request_attach(absorber: Entity, target: Entity) -> void:
